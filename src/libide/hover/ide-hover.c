...
#define GRACE_X 50
#define GRACE_Y 50
#define MOTION_SETTLE_TIMEOUT_MSEC 750
                                         ...
                                         IdeHoverPopover        *popover)
{
  ...
  g_assert (event != NULL);
  g_assert (IDE_IS_HOVER_POPOVER (popover));
  g_assert (self->state == IDE_HOVER_STATE_IN_POPOVER);
  ...
  if (self->providers == NULL)
    {
      ...
      if ((view = dzl_signal_group_get_target (self->signals)))
        {
          IdeContext *context = ide_widget_get_context (GTK_WIDGET (view));

          if (context != NULL)
            {
              self->providers =
                ide_extension_set_adapter_new (context,
