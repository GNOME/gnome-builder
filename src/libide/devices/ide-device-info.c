...
ide_device_info_finalize (GObject *object)
{
  ...
  g_clear_pointer (&self->machine_config_name, ide_machine_config_name_unref);
                              ...
                              GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_MACHINE_CONFIG_NAME:
      g_value_set_boxed (value, ide_device_info_get_machine_config_name (self));
      break;
                              ...
                              GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_MACHINE_CONFIG_NAME:
      ide_device_info_set_machine_config_name (self, g_value_get_boxed (value));
      break;
...
ide_device_info_class_init (IdeDeviceInfoClass *klass)
{
  ...
  properties [PROP_MACHINE_CONFIG_NAME] =
    g_param_spec_boxed ("machine-config-name",
                        ...
                        "The #IdeMachineConfigName object holding all the configuration name values",
...
{
  self->machine_config_name = ide_machine_config_name_new_from_system ();
}
...
/**
 * ide_device_info_get_machine_config_name:
 * @self: An #IdeDeviceInfo
 *
 * Get the #IdeMachineConfigName object describing the configuration name
 * of the Device (its architecture…)
 *
 * Returns: (transfer none) (nullable): An #IdeMachineConfigName.
 *
 * Since: 3.30
 */
IdeMachineConfigName *
ide_device_info_get_machine_config_name (IdeDeviceInfo *self)
{
  g_return_val_if_fail (IDE_IS_DEVICE_INFO (self), NULL);

  return self->machine_config_name;
}

/**
 * ide_device_info_set_machine_config_name:
 * @self: An #IdeDeviceInfo
 *
 * Set the #IdeMachineConfigName object describing the configuration name
 *
 * Since: 3.30
 */
void
ide_device_info_set_machine_config_name (IdeDeviceInfo         *self,
                                         IdeMachineConfigName  *machine_config_name)
{
  g_return_if_fail (IDE_IS_DEVICE_INFO (self));

  g_clear_pointer (&self->machine_config_name, ide_machine_config_name_unref);
  self->machine_config_name = ide_machine_config_name_ref (machine_config_name);
  g_object_notify_by_pspec (G_OBJECT (self), properties [PROP_MACHINE_CONFIG_NAME]);
}
