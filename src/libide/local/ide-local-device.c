...
ide_local_device_constructed (GObject *object)
{
  ...
  if (priv->triplet == NULL)
  ...
  else
    {
      ...
      vendor = ide_triplet_get_vendor (priv->triplet);
      if (vendor != NULL)
        device_vendor = g_strdup (vendor);
      else
        device_vendor = g_strdup (ide_triplet_get_vendor (triplet));

      kernel = ide_triplet_get_kernel (priv->triplet);
      if (kernel != NULL)
        device_kernel = g_strdup (kernel);
      else
        device_kernel = g_strdup (ide_triplet_get_kernel (triplet));

      operating_system = ide_triplet_get_operating_system (priv->triplet);
