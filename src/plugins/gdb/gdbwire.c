...
static int yy_get_next_buffer (yyscan_t yyscanner)
{
		...
		while ( num_to_read <= 0 )
			...
			if ( b->yy_is_our_buffer )
				...
				b->yy_ch_buf = (char *)
					...
					gdbwire_mi_realloc((void *) b->yy_ch_buf,(yy_size_t) (b->yy_buf_size + 2) ,yyscanner );
				}
			else
	...
	if ((int) (yyg->yy_n_chars + number_to_move) > YY_CURRENT_BUFFER_LVALUE->yy_buf_size) {
		...
		YY_CURRENT_BUFFER_LVALUE->yy_ch_buf = (char *) gdbwire_mi_realloc((void *) YY_CURRENT_BUFFER_LVALUE->yy_ch_buf,(yy_size_t) new_size ,yyscanner );
