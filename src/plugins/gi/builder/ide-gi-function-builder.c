...
G_DEFINE_TYPE (IdeGiFunctionBuilder, ide_gi_function_builder, IDE_TYPE_GI_PARSER_OBJECT)

/* This is called only for functions at the namespace level */
static void
ide_gi_function_builder_index (IdeGiParserObject *parse_object,
                               IdeGiParserResult *result,
                               gpointer           user_data)
{
  IdeGiFunctionBuilder *self = (IdeGiFunctionBuilder *)parse_object;
  ...
  ide_gi_parser_result_add_object_index (result, name, IDE_GI_BLOB_TYPE_FUNCTION, offset);
}

static void
ide_gi_function_builder_start_element (GMarkupParseContext  *context,
                                       ...
                                       GError              **error)
{
  IdeGiFunctionBuilder *self = (IdeGiFunctionBuilder *)user_data;
  IdeGiParserResult *result;
  IdeGiPool *pool;
  IdeGiParserObject *child;
  IdeGiElementType element_type;

  g_assert (IDE_IS_GI_FUNCTION_BUILDER (self));
  ...
  if (element_type & IDE_GI_PARSER_ELEMENT_MASK_FUNCTION)
    {
...
static void
ide_gi_function_builder_end_element (GMarkupParseContext  *context,
                                     const gchar          *element_name,
                                     gpointer              user_data,
                                     GError              **error)
{
...
static const GMarkupParser markup_parser = {
  ide_gi_function_builder_start_element,
  ide_gi_function_builder_end_element,
  NULL,
  NULL,
  NULL,
};
...
static gboolean
ide_gi_function_builder_parse (IdeGiParserObject    *parser_object,
                               ...
                               GError              **error)
{
  IdeGiFunctionBuilder *self = IDE_GI_FUNCTION_BUILDER (parser_object);
  ...
  g_assert (g_str_equal (element_name, "constructor") ||
            g_str_equal (element_name, "function") ||
            g_str_equal (element_name, "method") ||
            g_str_equal (element_name, "virtual-method"));
  ...
  if (!ide_gi_helper_markup_collect_attributes (result,
                                                ...
                                                IDE_GI_MARKUP_COLLECT_OFFSET32_STRING | IDE_GI_MARKUP_COLLECT_OPTIONAL, "", "shadowed-by", &self->blob.shadowed_by,
                                                IDE_GI_MARKUP_COLLECT_OFFSET32_STRING | IDE_GI_MARKUP_COLLECT_OPTIONAL, "", "shadows", &self->blob.shadows,
                                                IDE_GI_MARKUP_COLLECT_OFFSET32_STRING | IDE_GI_MARKUP_COLLECT_OPTIONAL, "", "moved-to", &self->blob.moved_to,
                                                IDE_GI_MARKUP_COLLECT_OFFSET32_STRING | IDE_GI_MARKUP_COLLECT_OPTIONAL, "", "invoker", &self->blob.invoker,
                                                IDE_GI_MARKUP_COLLECT_INVALID, NULL, NULL, NULL))
    return FALSE;

  if (g_str_equal (element_name, "constructor"))
    self->blob.common.blob_type = IDE_GI_BLOB_TYPE_CONSTRUCTOR;
  else if (g_str_equal (element_name, "function"))
    self->blob.common.blob_type = IDE_GI_BLOB_TYPE_FUNCTION;
  else if (g_str_equal (element_name, "method"))
    self->blob.common.blob_type = IDE_GI_BLOB_TYPE_METHOD;
  else if (g_str_equal (element_name, "virtual-method"))
    self->blob.common.blob_type = IDE_GI_BLOB_TYPE_VFUNC;
  else
...
static gpointer
ide_gi_function_builder_finish (IdeGiParserObject *parser_object)
{
  IdeGiFunctionBuilder *self = IDE_GI_FUNCTION_BUILDER (parser_object);
...
static void
ide_gi_function_builder_reset (IdeGiParserObject *parser_object)
{
  IdeGiFunctionBuilder *self = IDE_GI_FUNCTION_BUILDER (parser_object);

  memset (self + sizeof (IdeGiParserObject),
          0,
          sizeof (IdeGiFunctionBuilder) - sizeof (IdeGiParserObject));
}

IdeGiParserObject *
ide_gi_function_builder_new (void)
{
  IdeGiParserObject *self;

  self = IDE_GI_PARSER_OBJECT (g_object_new (IDE_TYPE_GI_FUNCTION_BUILDER, NULL));

  _ide_gi_parser_object_set_element_type (self, IDE_GI_ELEMENT_TYPE_FUNCTION);
  return self;
}

static void
ide_gi_function_builder_finalize (GObject *object)
{
  G_OBJECT_CLASS (ide_gi_function_builder_parent_class)->finalize (object);
}

static void
ide_gi_function_builder_class_init (IdeGiFunctionBuilderClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  IdeGiParserObjectClass *parent_class = IDE_GI_PARSER_OBJECT_CLASS (klass);

  object_class->finalize = ide_gi_function_builder_finalize;

  parent_class->finish = ide_gi_function_builder_finish;
  parent_class->index = ide_gi_function_builder_index;
  parent_class->parse = ide_gi_function_builder_parse;
  parent_class->reset = ide_gi_function_builder_reset;
}

static void
ide_gi_function_builder_init (IdeGiFunctionBuilder *self)
{
}
