...
G_DEFINE_TYPE (IdeGiAliasBuilder, ide_gi_alias_builder, IDE_TYPE_GI_PARSER_OBJECT)

static void
ide_gi_alias_builder_index (IdeGiParserObject *parse_object,
                            IdeGiParserResult *result,
                            gpointer           user_data)
{
  IdeGiAliasBuilder *self = (IdeGiAliasBuilder *)parse_object;
  ...
  ide_gi_parser_result_add_object_index (result, name, IDE_GI_BLOB_TYPE_ALIAS, offset);
}

static void
ide_gi_alias_builder_start_element (GMarkupParseContext  *context,
                                    ...
                                    GError              **error)
{
  IdeGiAliasBuilder *self = (IdeGiAliasBuilder *)user_data;
  IdeGiParserResult *result;
  IdeGiPool *pool;
  IdeGiParserObject *child;
  IdeGiElementType element_type;

  g_assert (IDE_IS_GI_ALIAS_BUILDER (self));
  ...
  if (element_type & IDE_GI_PARSER_ELEMENT_MASK_ALIAS)
    {
...
static void
ide_gi_alias_builder_end_element (GMarkupParseContext  *context,
                                  const gchar          *element_name,
                                  gpointer              user_data,
                                  GError              **error)
{
...
static const GMarkupParser markup_parser = {
  ide_gi_alias_builder_start_element,
  ide_gi_alias_builder_end_element,
  NULL,
  NULL,
  NULL,
};
...
static gboolean
ide_gi_alias_builder_parse (IdeGiParserObject    *parser_object,
                            ...
                            GError              **error)
{
  IdeGiAliasBuilder *self = IDE_GI_ALIAS_BUILDER (parser_object);
  ...
  g_assert (g_str_equal (element_name, "alias"));
  ...
  self->blob.common.blob_type = IDE_GI_BLOB_TYPE_ALIAS;
...
static gpointer
ide_gi_alias_builder_finish (IdeGiParserObject *parser_object)
{
  IdeGiAliasBuilder *self = IDE_GI_ALIAS_BUILDER (parser_object);
...
static void
ide_gi_alias_builder_reset (IdeGiParserObject *parser_object)
{
  IdeGiAliasBuilder *self = IDE_GI_ALIAS_BUILDER (parser_object);

  memset (self + sizeof (IdeGiParserObject),
          0,
          sizeof (IdeGiAliasBuilder) - sizeof (IdeGiParserObject));
}

IdeGiParserObject *
ide_gi_alias_builder_new (void)
{
  IdeGiParserObject *self;

  self = IDE_GI_PARSER_OBJECT (g_object_new (IDE_TYPE_GI_ALIAS_BUILDER, NULL));

  _ide_gi_parser_object_set_element_type (self, IDE_GI_ELEMENT_TYPE_ALIAS);
  return self;
}

static void
ide_gi_alias_builder_finalize (GObject *object)
{
  G_OBJECT_CLASS (ide_gi_alias_builder_parent_class)->finalize (object);
}

static void
ide_gi_alias_builder_class_init (IdeGiAliasBuilderClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  IdeGiParserObjectClass *parent_class = IDE_GI_PARSER_OBJECT_CLASS (klass);

  object_class->finalize = ide_gi_alias_builder_finalize;

  parent_class->finish = ide_gi_alias_builder_finish;
  parent_class->index = ide_gi_alias_builder_index;
  parent_class->parse = ide_gi_alias_builder_parse;
  parent_class->reset = ide_gi_alias_builder_reset;
}

static void
ide_gi_alias_builder_init (IdeGiAliasBuilder *self)
{
}
