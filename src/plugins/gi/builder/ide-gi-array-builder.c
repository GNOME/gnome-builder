...
static const TypeInfo ARRAY_TYPES_INFO [] =
{
  { "GLib.Array"      , IDE_GI_BASIC_TYPE_G_ARRAY       },
  { "GLib.PtrArray"   , IDE_GI_BASIC_TYPE_G_PTR_ARRAY   },
  { "GLib.BytesArray" , IDE_GI_BASIC_TYPE_G_BYTES_ARRAY },
};

G_DEFINE_TYPE (IdeGiArrayBuilder, ide_gi_array_builder, IDE_TYPE_GI_PARSER_OBJECT)

static IdeGiBasicType
get_array_type (IdeGiArrayBuilder *self,
                const gchar       *name)
{
  for (guint i = 0; i < G_N_ELEMENTS (ARRAY_TYPES_INFO); i++)
    {
      if (g_strcmp0 (name, ARRAY_TYPES_INFO[i].name) == 0)
        return ARRAY_TYPES_INFO[i].type;
    }

  return IDE_GI_BASIC_TYPE_C_ARRAY;
}

static void
ide_gi_array_builder_start_element (GMarkupParseContext  *context,
                                    ...
                                    GError              **error)
{
  IdeGiArrayBuilder *self = (IdeGiArrayBuilder *)user_data;
  IdeGiParserResult *result;
  IdeGiPool *pool;
  IdeGiParserObject *child;
  IdeGiElementType element_type;

  g_assert (IDE_IS_GI_ARRAY_BUILDER (self));
  ...
  if (element_type & IDE_GI_PARSER_ELEMENT_MASK_ARRAY)
    {
...
static void
ide_gi_array_builder_end_element (GMarkupParseContext  *context,
                                  const gchar          *element_name,
                                  gpointer              user_data,
                                  GError              **error)
{
...
static const GMarkupParser markup_parser = {
  ide_gi_array_builder_start_element,
  ide_gi_array_builder_end_element,
  NULL,
  NULL,
  NULL,
};
...
static gboolean
ide_gi_array_builder_parse (IdeGiParserObject    *parser_object,
                            ...
                            GError              **error)
{
  IdeGiArrayBuilder *self = IDE_GI_ARRAY_BUILDER (parser_object);
  const gchar *name;
  ...
  if (!ide_gi_helper_markup_collect_attributes (result,
                                                ...
                                                IDE_GI_MARKUP_COLLECT_BOOLEAN | IDE_GI_MARKUP_COLLECT_OPTIONAL, "0", "zero-terminated", &zero_terminated,
                                                ...
                                                IDE_GI_MARKUP_COLLECT_INT64 | IDE_GI_MARKUP_COLLECT_OPTIONAL, "-1", "fixed-size", &fixed_size,
                                                IDE_GI_MARKUP_COLLECT_INT64 | IDE_GI_MARKUP_COLLECT_OPTIONAL, "-1", "length", &length,
                                                IDE_GI_MARKUP_COLLECT_INVALID, NULL, NULL, NULL))
    return FALSE;

  self->blob.common.blob_type = IDE_GI_BLOB_TYPE_ARRAY;
  self->blob.array_type = get_array_type (self, name);
  self->blob.common.name = ide_gi_parser_result_add_string (result, name);

  self->blob.common.introspectable = introspectable;
  self->blob.zero_terminated = zero_terminated;

  if (fixed_size > -1)
    {
      self->blob.size = fixed_size;
      self->blob.has_size = TRUE;
    }

  if (length  > -1)
    {
      self->blob.length = length;
      self->blob.has_length = TRUE;
    }
...
static gpointer
ide_gi_array_builder_finish (IdeGiParserObject *parser_object)
{
  IdeGiArrayBuilder *self = IDE_GI_ARRAY_BUILDER (parser_object);
...
static void
ide_gi_array_builder_reset (IdeGiParserObject *parser_object)
{
  IdeGiArrayBuilder *self = IDE_GI_ARRAY_BUILDER (parser_object);

  memset (self + sizeof (IdeGiParserObject),
          0,
          sizeof (IdeGiArrayBuilder) - sizeof (IdeGiParserObject));
}

IdeGiParserObject *
ide_gi_array_builder_new (void)
{
  IdeGiParserObject *self;

  self = IDE_GI_PARSER_OBJECT (g_object_new (IDE_TYPE_GI_ARRAY_BUILDER, NULL));

  _ide_gi_parser_object_set_element_type (self, IDE_GI_ELEMENT_TYPE_ARRAY);
  return self;
}

static void
ide_gi_array_builder_finalize (GObject *object)
{
  G_OBJECT_CLASS (ide_gi_array_builder_parent_class)->finalize (object);
}

static void
ide_gi_array_builder_class_init (IdeGiArrayBuilderClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  IdeGiParserObjectClass *parent_class = IDE_GI_PARSER_OBJECT_CLASS (klass);

  object_class->finalize = ide_gi_array_builder_finalize;

  parent_class->finish = ide_gi_array_builder_finish;
  parent_class->parse = ide_gi_array_builder_parse;
  parent_class->reset = ide_gi_array_builder_reset;
}

static void
ide_gi_array_builder_init (IdeGiArrayBuilder *self)
{
}
