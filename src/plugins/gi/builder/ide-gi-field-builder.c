...
G_DEFINE_TYPE (IdeGiFieldBuilder, ide_gi_field_builder, IDE_TYPE_GI_PARSER_OBJECT)

static void
ide_gi_field_builder_start_element (GMarkupParseContext  *context,
                                    ...
                                    GError              **error)
{
  IdeGiFieldBuilder *self = (IdeGiFieldBuilder *)user_data;
  IdeGiParserResult *result;
  IdeGiPool *pool;
  IdeGiParserObject *child;
  IdeGiElementType element_type;

  g_assert (IDE_IS_GI_FIELD_BUILDER (self));
  ...
  if (element_type & IDE_GI_PARSER_ELEMENT_MASK_FIELD)
    {
...
static void
ide_gi_field_builder_end_element (GMarkupParseContext  *context,
                                  const gchar          *element_name,
                                  gpointer              user_data,
                                  GError              **error)
{
  ...
  if (element_type & IDE_GI_PARSER_ELEMENT_MASK_FIELD)
    {
      if (element_type & IDE_GI_PARSER_ELEMENT_MASK_DOC)
      ...
      else if (element_type == IDE_GI_ELEMENT_TYPE_CALLBACK)
        {
          ...
          self->blob.type_ref = ide_gi_parser_result_add_callback (result, blob);
        }
...
static const GMarkupParser markup_parser = {
  ide_gi_field_builder_start_element,
  ide_gi_field_builder_end_element,
  NULL,
  NULL,
  NULL,
};
...
static gboolean
ide_gi_field_builder_parse (IdeGiParserObject    *parser_object,
                            ...
                            GError              **error)
{
  IdeGiFieldBuilder *self = IDE_GI_FIELD_BUILDER (parser_object);
  ...
  if (!ide_gi_helper_markup_collect_attributes (result,
                                                ...
                                                IDE_GI_MARKUP_COLLECT_BOOLEAN | IDE_GI_MARKUP_COLLECT_OPTIONAL, "0", "private", &private,
                                                IDE_GI_MARKUP_COLLECT_STABILITY | IDE_GI_MARKUP_COLLECT_OPTIONAL, "Stable", "stability", &stability,
                                                IDE_GI_MARKUP_COLLECT_UINT64 | IDE_GI_MARKUP_COLLECT_OPTIONAL, "0", "bits", &self->blob.bits,
...
static gpointer
ide_gi_field_builder_finish (IdeGiParserObject *parser_object)
{
  IdeGiFieldBuilder *self = IDE_GI_FIELD_BUILDER (parser_object);
...
static void
ide_gi_field_builder_reset (IdeGiParserObject *parser_object)
{
  IdeGiFieldBuilder *self = IDE_GI_FIELD_BUILDER (parser_object);

  memset (self + sizeof (IdeGiParserObject),
          0,
          sizeof (IdeGiFieldBuilder) - sizeof (IdeGiParserObject));
}

IdeGiParserObject *
ide_gi_field_builder_new (void)
{
  IdeGiParserObject *self;

  self = IDE_GI_PARSER_OBJECT (g_object_new (IDE_TYPE_GI_FIELD_BUILDER, NULL));

  _ide_gi_parser_object_set_element_type (self, IDE_GI_ELEMENT_TYPE_FIELD);
  return self;
}

static void
ide_gi_field_builder_finalize (GObject *object)
{
  G_OBJECT_CLASS (ide_gi_field_builder_parent_class)->finalize (object);
}

static void
ide_gi_field_builder_class_init (IdeGiFieldBuilderClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  IdeGiParserObjectClass *parent_class = IDE_GI_PARSER_OBJECT_CLASS (klass);

  object_class->finalize = ide_gi_field_builder_finalize;

  parent_class->finish = ide_gi_field_builder_finish;
  parent_class->parse = ide_gi_field_builder_parse;
  parent_class->reset = ide_gi_field_builder_reset;
}

static void
ide_gi_field_builder_init (IdeGiFieldBuilder *self)
{
}
