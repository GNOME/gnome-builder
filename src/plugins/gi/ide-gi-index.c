  ...
  if (--self->pool_count == 0 && self->pool_all_pushed)
    {
      ...
      ide_task_set_task_data (end_task, g_steal_pointer (&state), update_state_free);
  ...
  if (!self->is_updating &&
      NULL != (next_update_task = g_queue_peek_head (self->update_queue)))
    {
      ...
      start_task = ide_task_new (self,
                                 ...
                                 update_async_cb1,
                                 state);
      ide_task_set_task_data (start_task, state, (GDestroyNotify)NULL);
                           ...
                           gpointer             user_data)
{
  ...
  ide_task_set_task_data (update_task, state, update_state_free);
