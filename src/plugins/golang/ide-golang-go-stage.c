...
G_DEFINE_TYPE (IdeGolangGoStage, ide_golang_go_stage, IDE_TYPE_PIPELINE_STAGE)
                 ...
                 GError                **error)
{
  ...
  if (NULL == (goroot = ide_config_getenv (config, "GOROOT")))
    {
      g_set_error (error,
                   G_IO_ERROR,
                   G_IO_ERROR_FAILED,
  ...
  if (NULL == (gopath = ide_config_getenv (config, "GOPATH")))
    {
      g_set_error (error,
                   G_IO_ERROR,
                   G_IO_ERROR_FAILED,
...
static void
ide_golang_go_stage_build_async (IdePipelineStage       *stage,
                                   IdePipeline    *pipeline,
                                   GCancellable        *cancellable,
                                   ...
                                   gpointer             user_data)
{
  ...
  ide_task_set_source_tag (task, ide_golang_go_stage_build_async);
...
static gboolean
ide_golang_go_stage_build_finish (IdePipelineStage  *stage,
                                    GAsyncResult   *result,
                                    GError        **error)
{
  gboolean ret;
...
static void
ide_golang_go_stage_clean_async (IdePipelineStage       *stage,
                                 IdePipeline    *pipeline,
                                 GCancellable        *cancellable,
                                 GAsyncReadyCallback  callback,
                                 gpointer             user_data)
...
static gboolean
ide_golang_go_stage_clean_finish (IdePipelineStage  *stage,
                                  GAsyncResult   *result,
                                  GError        **error)
{
  gboolean ret;
...
static void
ide_golang_go_stage_query (IdePipelineStage    *stage,
                           IdePipeline *pipeline,
                           GPtrArray        *targets,
                           GCancellable     *cancellable)
{
...
ide_golang_go_stage_class_init (IdeGolangGoStageClass *klass)
{
  ...
  build_stage_class->build_async = ide_golang_go_stage_build_async;
  build_stage_class->build_finish = ide_golang_go_stage_build_finish;
