                         ...
                         GFile        *file)
{
  ...
  system = g_key_file_get_string (keyfile, "host_machine", "system", &read_error);
  if (system != NULL)
    system = g_utf8_substring (system, 1, g_utf8_strlen (system, -1) - 1);
  ...
  for (int i = 0; binaries[i] != NULL; i++)
  {
    ...
    exec_path = g_key_file_get_string (keyfile, "binaries", lang, &key_error);
    if (path != NULL)
      exec_path = g_utf8_substring (system, 1, g_utf8_strlen (exec_path, -1) - 1);
