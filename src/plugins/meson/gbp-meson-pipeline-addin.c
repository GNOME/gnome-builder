...
static gchar *
_gbp_meson_get_quoted_path (const gchar *unquoted_path)
{
    g_return_val_if_fail (unquoted_path != NULL, NULL);

    return g_strconcat ("'", unquoted_path, "'", NULL);
}
...
add_lang_executable (gchar *lang,
                     ...
                     GKeyFile *keyfile)
{
    g_autofree gchar *quoted_path = _gbp_meson_get_quoted_path (path);
    g_key_file_set_string (keyfile, "binaries", lang, quoted_path);
}
                               ...
                               IdeBuildPipeline      *pipeline)
{
  ...
  if (g_strcmp0 (ide_toolchain_get_id (toolchain), "default") != 0) {
    ...
    quoted_path = _gbp_meson_get_quoted_path (ide_toolchain_get_archiver (toolchain));
    if (quoted_path != NULL)
      g_key_file_set_string (crossbuild_keyfile, "binaries", "ar", quoted_path);

    quoted_path = _gbp_meson_get_quoted_path (ide_toolchain_get_strip (toolchain));
    if (quoted_path != NULL)
      g_key_file_set_string (crossbuild_keyfile, "binaries", "strip", quoted_path);

    quoted_path = _gbp_meson_get_quoted_path (ide_toolchain_get_pkg_config (toolchain));
    if (quoted_path != NULL)
      g_key_file_set_string (crossbuild_keyfile, "binaries", "pkgconfig", quoted_path);

    quoted_path = _gbp_meson_get_quoted_path (ide_toolchain_get_exe_wrapper (toolchain));
    if (quoted_path != NULL)
      g_key_file_set_string (crossbuild_keyfile, "binaries", "exe_wrapper", quoted_path);
