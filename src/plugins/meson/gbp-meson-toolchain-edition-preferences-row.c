      ...
      if (!g_file_move (source, destination, G_FILE_COPY_NONE, NULL, NULL, NULL, &error))
        {
          ...
          g_critical ("Unable to rename file: %s", error->message);
          return;
        }
                                                      ...
                                                      gpointer                                user_data)
{
  ...
  if (!g_key_file_load_from_file (keyfile, self->toolchain_path, G_KEY_FILE_KEEP_COMMENTS|G_KEY_FILE_KEEP_TRANSLATIONS, &error))
    {
      g_critical ("Unable to load file \"%s\": %s", self->toolchain_path, error->message);
      return;
    }
  ...
  if (!g_key_file_save_to_file (keyfile, self->toolchain_path, &error))
    {
      g_critical ("Unable to remove tool: %s", error->message);
      return;
    }
}
                                                ...
                                                gpointer                                user_data)
{
  ...
  if (!g_file_delete (file, NULL, &error))
    {
      g_critical ("Error removing \"%s\": %s", self->toolchain_path, error->message);
      return;
    }
