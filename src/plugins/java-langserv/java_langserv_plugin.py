...
class JavaService(Ide.Object, Ide.Service):
    ...
    * Also we probably need more error checking here, perhaps we could push this logig
    ...
    def check_output(self, args):
        ...
        return process.communicate_utf8(None, None)

    def locate_java(self):
        result = self.check_output(['which', 'java']).stdout_buf
        path = result.rstrip()
        if (os.path.exists(path) is None):
            self.log("Couldn't locate java. Exiting..")
            return None
        self.log("Found OpenJdk at: {path}".format(path=path))
        return path

    def get_java_version(self, java_path):
        output = self.check_output([java_path, '-version']).stderr_buf
        begin_idx = output.find('"')+1
        end_idx = output.find('"', begin_idx+1)

        if (begin_idx == end_idx):
            self.log("Couldn't get java version")
            return 0

        version_string = output[begin_idx:end_idx]
